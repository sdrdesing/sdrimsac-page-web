...
class CarritoModel {
    ...
    public function obtenerPorProducto($id_producto) {
        ...
        if ($usuario_id !== null) {
            $stmt = $this->conn->prepare("SELECT * FROM carrito WHERE id_producto = ? AND usuario_id = ?");
    ...
    public function agregarProducto($id_producto, $nombre, $precio, $imagen, $cantidad, $stock) {
        ...
        if($existente && $usuario_id !== null) {
            $cantidad_total = $existente['cantidad'] + $cantidad;
            if($cantidad_total > $stock) $cantidad_total = $stock;
            $stmt = $this->conn->prepare("UPDATE carrito SET cantidad = ? WHERE id_producto = ? AND usuario_id = ?");
            $stmt->bind_param("iii", $cantidad_total, $id_producto, $usuario_id);
            $stmt->execute();
        } else if($usuario_id !== null) {
            if($cantidad > $stock) $cantidad = $stock;
            $stmt = $this->conn->prepare("INSERT INTO carrito (usuario_id, id_producto, nombre_producto, precio, imagen, cantidad) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("iisdsi", $usuario_id, $id_producto, $nombre, $precio, $imagen, $cantidad);
            $stmt->execute();
        } else {
